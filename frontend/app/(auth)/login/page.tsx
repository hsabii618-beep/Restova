  ...
  return (
    <AuthLayout
      ...
      subtitle={
        <>
          New to Restova?{' '}
          <Link href="/signup" style={{ color: 'var(--color-brand-primary)', fontWeight: 600 }}>Create an account</Link>
        </>
      }
    >
      {error && (
      ...
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: '24px' }}>
        ...
        <div style={{ padding: '0 12px', fontSize: '14px', color: 'var(--color-text-muted)' }}>or continue with email</div>
        <div style={{ flex: 1, height: '1px', background: 'var(--color-border-light)' }}></div>
      </div>
      ...
      <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <label htmlFor="password" style={{ fontSize: '14px', fontWeight: 600, color: 'var(--color-text-primary)' }}>Password</label>
            <Link href="/forgot-password" style={{ fontSize: '13px', color: 'var(--color-brand-primary)', fontWeight: 500 }}>Forgot password?</Link>
          </div>
          <input
            id="password"
            type="password"
