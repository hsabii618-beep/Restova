...
-- Create core tables for the Restova database (Phase 1)
...
CREATE TABLE IF NOT EXISTS public.orders (
    ...
    -- Phase 1 patch: until next_display_number() is introduced, allow inserts without computed numbering
    ...
    -- Phase 1 patch: default expiry (business rule) even before scheduler exists
...
CREATE TABLE IF NOT EXISTS public.order_items (
    ...
    -- Patch: item_id is nullable to support 'ON DELETE SET NULL' while preserving order history
...
-- Idempotent triggers (create only if missing)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger
        WHERE tgname = 'set_restaurants_updated_at'
    ) THEN
